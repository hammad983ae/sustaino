-- Enable RLS on tables that don't have it and add missing policies

-- Enable RLS on property_addresses table
ALTER TABLE public.property_addresses ENABLE ROW LEVEL SECURITY;

-- Add RLS policy for property_addresses (public read access for address lookup)
CREATE POLICY "Property addresses are publicly readable"
ON public.property_addresses
FOR SELECT
USING (true);

-- Add RLS policy for monthly_update_notifications (admin only write)
...
-- Add missing RLS policies for tables with foreign key relationships
