...
-- Create function to update ownership percentages
CREATE OR REPLACE FUNCTION public.update_ownership_percentage()
RETURNS TRIGGER AS $$
BEGIN
  -- Update ownership percentage for the specific investment
  UPDATE public.pool_investments 
  SET ownership_percentage = CASE 
    WHEN NEW.investment_amount > 0 AND 
         (SELECT current_pool_size FROM public.investment_pools WHERE id = NEW.pool_id) > 0 
    THEN (NEW.investment_amount / (SELECT current_pool_size FROM public.investment_pools WHERE id = NEW.pool_id)) * 100
    ELSE 0
  END
  WHERE id = NEW.id;
...
-- Create trigger to automatically update ownership percentage
CREATE TRIGGER update_pool_investment_ownership
  AFTER INSERT OR UPDATE ON public.pool_investments
  FOR EACH ROW
  EXECUTE FUNCTION public.update_ownership_percentage();

-- Create function to update pool size and investor count
CREATE OR REPLACE FUNCTION public.update_pool_totals()
RETURNS TRIGGER AS $$
BEGIN
  -- Update pool totals when investments change
  IF TG_OP = 'INSERT' THEN
    UPDATE public.investment_pools 
    SET 
      current_pool_size = current_pool_size + NEW.investment_amount,
      number_of_investors = (
        SELECT COUNT(DISTINCT investor_id) 
        FROM public.pool_investments 
        WHERE pool_id = NEW.pool_id AND investment_status = 'active'
      )
    WHERE id = NEW.pool_id;
    
    -- Update raised amount in investment property
    UPDATE public.investment_properties 
    SET raised_amount = raised_amount + NEW.investment_amount
...
-- Create trigger to update pool totals
CREATE TRIGGER update_investment_pool_totals
  AFTER INSERT ON public.pool_investments
  FOR EACH ROW
  EXECUTE FUNCTION public.update_pool_totals();