...
const PlanningDataIntegration = ({ propertyAddress = "", onDataFetched }: PlanningDataIntegrationProps) => {
  ...
  useEffect(() => {
    ...
    if (fullAddress && fullAddress !== searchAddress) {
      ...
      // Force clear ALL localStorage data that might interfere
      if (typeof Storage !== 'undefined') {
        ...
        keys.forEach(key => {
          if (key.includes('planning') || 
              ...
              key.includes('search')) {
            console.log('PlanningDataIntegration clearing:', key);
            localStorage.removeItem(key);
          }
        });
      }
      
      // Force refresh all planning components
      setTimeout(() => {
        const event = new CustomEvent('planningDataCleared', { 
          detail: { 
            address: fullAddress, 
            timestamp: Date.now() 
          } 
