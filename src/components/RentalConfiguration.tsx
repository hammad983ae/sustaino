    ...
    return (
      <div className="space-y-2">
        ...
        <Popover>
          ...
          <PopoverContent className="w-full p-0 z-50 bg-background border">
    ...
    <Card className="w-full max-w-4xl mx-auto">
      <CardHeader>
        <div className="flex items-center gap-2">
          ...
          <CardTitle className="text-xl font-semibold">Rental Valuation Configuration</CardTitle>
        </div>
      </CardHeader>
      <CardContent className="space-y-6">
        {/* Rental Assessment Details */}
        <div className="space-y-4">
          <h3 className="font-semibold text-lg">Rental Assessment Details</h3>
          <p className="text-sm text-muted-foreground">Configure rental valuation settings and assessment parameters</p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              <Label htmlFor="rental-gst-treatment" className="text-sm font-medium">GST Treatment</Label>
              <Select value={formData['rental-gst-treatment'] || ''} onValueChange={(value) => handleSelectChange('rental-gst-treatment', value)}>
                <SelectTrigger id="rental-gst-treatment" className="mt-1">
        ...
        {/* Incentives and Additional Configuration */}
        <div className="space-y-4">
          <h3 className="font-semibold text-lg">Incentives and Additional Configuration</h3>
        ...
        <div className="bg-green-50 border border-green-200 rounded-lg p-4">
          <h4 className="font-medium text-green-900 mb-2">Rental Configuration Summary</h4>
          <div className="text-sm text-green-800 space-y-1">
            <p>• Assessment Type: {formData['rental-assessment-type'] ? formData['rental-assessment-type'].replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Not selected'}</p>
            <p>• Basis of Rent: {formData['rental-basis'] ? formData['rental-basis'].replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Not selected'}</p>
            <p>• GST Treatment: {formData['rental-gst-treatment'] ? formData['rental-gst-treatment'].replace('-', ' ').replace(/\b\w/g, l => l.toUpperCase()) : 'Not selected'}</p>
            <p>• Incentives: {(selectedValues['Incentive Types'] || []).length > 0 ? (selectedValues['Incentive Types'] || []).join(', ') : 'None selected'}</p>
          </div>
        </div>
      </CardContent>
    </Card>
...
export default RentalConfiguration;